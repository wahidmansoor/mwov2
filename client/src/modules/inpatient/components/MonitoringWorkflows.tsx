...
import { useMonitoringWorkflowsQ, filterByEvidence, searchRows, useBookmarks } from "@/lib/hooks/useInpatientData";
import { CopyLinkButton, GlobalSearch, EvidenceChips, PrintSectionButton, DemoBanner } from "./_shared";

export function MonitoringWorkflows() {
  const { data, isLoading } = useMonitoringWorkflowsQ();
    ...
    <div className="space-y-4" id="monitoring">
      ...
      <div className="flex items-center justify-between gap-3">
        <Alert className="flex-1">
          <AlertDescription>Daily assessment guides and clinical monitoring parameters.</AlertDescription>
        </Alert>
        <div className="flex gap-2">
          <PrintSectionButton containerId="monitoring-container" />
      ...
      {bookmarkedSlugs.size > 0 && (
        <Card>
          ...
          <CardContent className="grid gap-2">
            {rows.filter(r => bookmarkedSlugs.has(`monitoring-${r.slug}`)).map(r => (
              <a key={r.id} className="text-sm underline" href={`#monitoring-${r.slug}`}>{r.title}</a>
            ))}
          </CardContent>
        </Card>
      )}

      <div id="monitoring-container" className="space-y-4">
        {(rows || []).map((item) => (
          <MonitoringItem key={item.id} item={item} collapseAll={collapseAll} onBookmark={() => toggle(`monitoring-${item.slug}`)} isBookmarked={isBookmarked(`monitoring-${item.slug}`)} />
        ))}
...
function MonitoringItem({ item, collapseAll, onBookmark, isBookmarked }: any) {
  const [open, setOpen] = useState(!collapseAll);
  React.useEffect(()=> setOpen(!collapseAll), [collapseAll]);

  return (
    <Card id={`monitoring-${item.slug}`}>
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle className="flex items-center gap-2">
          ...
          {item.title}
          <Badge variant="secondary" className="ml-2">{item.frequency}</Badge>
        ...
        <div className="flex gap-2">
          ...
          <CopyLinkButton anchor={`monitoring-${item.slug}`} />
        </div>
      </CardHeader>
      {open && (
        <CardContent className="space-y-3">
          <Section label="Parameters to monitor" items={item.parameters} />
          <Section label="Escalation rules" items={item.escalation_rules} />
        </CardContent>
      )}
    </Card>
  );
